    ...
    while (res == SPIFFS_OK && entry_count > 0) {
        ...
        while (res == SPIFFS_OK && obj_lookup_page < (int) SPIFFS_OBJ_LOOKUP_PAGES(fs)) {
            ...
            res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_READ,
                             0, cur_block_addr + SPIFFS_PAGE_TO_PADDR(fs, obj_lookup_page), SPIFFS_CFG_LOG_PAGE_SZ(fs),
                             fs->lu_work);
            // check each entry
            while (res == SPIFFS_OK &&
                    ...
                    if (v) {
                        ...
                        if (res == SPIFFS_VIS_COUNTINUE || res == SPIFFS_VIS_COUNTINUE_RELOAD) {
                            if (res == SPIFFS_VIS_COUNTINUE_RELOAD) {
                                res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_READ,
                                                 0, cur_block_addr + SPIFFS_PAGE_TO_PADDR(fs, obj_lookup_page),
...
int32_t spiffs_page_allocate_data(
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_UPDT,
                     0, SPIFFS_BLOCK_TO_PADDR(fs, bix) + entry * sizeof(spiffs_obj_id), sizeof(spiffs_obj_id),
                     (uint8_t *) &obj_id);
    SPIFFS_CHECK_RES(res);
...
int32_t spiffs_page_move(
    ...
    if (page_data) {
        ...
        // copy page data
        res = spiffs_phys_cpy(fs, fh, SPIFFS_PAGE_TO_PADDR(fs, free_pix), SPIFFS_PAGE_TO_PADDR(fs, src_pix),
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_UPDT,
                     0, SPIFFS_BLOCK_TO_PADDR(fs, SPIFFS_BLOCK_FOR_PAGE(fs, free_pix)) +
                        SPIFFS_OBJ_LOOKUP_ENTRY_FOR_PAGE(fs, free_pix) * sizeof(spiffs_page_ix),
...
int32_t spiffs_page_delete(
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_DELE,
                     ...
                     SPIFFS_OBJ_LOOKUP_ENTRY_FOR_PAGE(fs, pix) * sizeof(spiffs_page_ix),
                     sizeof(spiffs_obj_id),
                     (uint8_t *) &d_obj_id);
    SPIFFS_CHECK_RES(res);
...
int32_t spiffs_object_create(
    ...
    // update page
    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT,
                     0, SPIFFS_OBJ_LOOKUP_ENTRY_TO_PADDR(fs, bix, entry), sizeof(spiffs_page_object_ix_header),
                     (uint8_t *) &oix_hdr);

    SPIFFS_CHECK_RES(res);
    spiffs_cb_object_event(fs, 0, SPIFFS_EV_IX_NEW, obj_id, 0, SPIFFS_OBJ_LOOKUP_ENTRY_TO_PIX(fs, bix, entry),
...
int32_t spiffs_object_update_index_hdr(
    ...
    res = spiffs_page_move(fs, fd == 0 ? 0 : fd->file_nbr, (uint8_t *) objix_hdr, obj_id, 0, objix_hdr_pix,
                           &new_objix_hdr_pix);
    ...
    if (res == SPIFFS_OK) {
        ...
        spiffs_cb_object_event(fs, fd, SPIFFS_EV_IX_UPD, obj_id, objix_hdr->p_hdr.span_ix, new_objix_hdr_pix,
                               objix_hdr->size);
        ...
        if (spix == 0) {
            if (ev == SPIFFS_EV_IX_NEW || ev == SPIFFS_EV_IX_UPD) {
                SPIFFS_DBG("       callback: setting fd %i:%04x objix_hdr_pix to %04x, size:%i\n", cur_fd->file_nbr,
                           cur_fd->obj_id, new_pix, new_size);
...
int32_t spiffs_object_open_by_page(
    ...
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_READ,
                     fd->file_nbr, SPIFFS_PAGE_TO_PADDR(fs, pix), sizeof(spiffs_page_object_ix_header),
                     (uint8_t *) &oix_hdr);
    SPIFFS_CHECK_RES(res);
...
int32_t spiffs_object_append(spiffs_fd *fd, u32_t offset, uint8_t *data, u32_t len) {
    spiffs *fs = fd->fs;
    int32_t res = SPIFFS_OK;
    u32_t written = 0;
    ...
    while (res == SPIFFS_OK && written < len) {
        ...
        if (cur_objix_spix != prev_objix_spix) {
                ...
                if (prev_objix_spix == 0) {
                    ...
                    if (offset == 0) {
                        ...
                        res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id,
                                                             fd->objix_hdr_pix, fs->work, 0, offset + written,
                                                             &new_objix_hdr_page);
                        SPIFFS_CHECK_RES(res);
                    ...
                    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_UPDT,
                                     fd->file_nbr, SPIFFS_PAGE_TO_PADDR(fs, cur_objix_pix), SPIFFS_CFG_LOG_PAGE_SZ(fs),
                                     fs->work);
                    SPIFFS_CHECK_RES(res);
                    spiffs_cb_object_event(fs, fd, SPIFFS_EV_IX_UPD, fd->obj_id, objix->p_hdr.span_ix, cur_objix_pix,
                                           0);
                    // update length in object index header page
                    res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id,
                                                         fd->objix_hdr_pix, 0, 0, offset + written,
                                                         &new_objix_hdr_page);
                    SPIFFS_CHECK_RES(res);
            ...
            if (cur_objix_spix == 0) {
                ...
                spiffs_span_ix len_objix_spix = SPIFFS_OBJ_IX_ENTRY_SPAN_IX(fs,
                                                                            (fd->size - 1) / SPIFFS_DATA_PAGE_SIZE(fs));
        ...
        if (page_offs == 0) {
            ...
            if (cur_objix_spix == 0) {
                ...
                data_page = ((spiffs_page_ix *) ((uint8_t *) objix + sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(
                        fs, data_spix)];
            }
            ...
            SPIFFS_DBG("append: %04x store to existing data page, %04x:%04x offset:%i, len %i, written %i\n",
                       fd->obj_id, data_page, data_spix, page_offs, to_write, written);
        }
        ...
        if (cur_objix_spix == 0) {
            ...
            ((spiffs_page_ix *) ((uint8_t *) objix + sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(fs,
                                                                                                      data_spix)] = data_page;
    ...
    if (cur_objix_spix != 0) {
        ...
        if (offset == 0) {
            ...
            SPIFFS_CHECK_RES(res2);
            // callback on object index update
            spiffs_cb_object_event(fs, fd, SPIFFS_EV_IX_UPD, fd->obj_id, objix_hdr->p_hdr.span_ix, cur_objix_pix,
                                   objix_hdr->size);
        } else {
...
// Modify object
// keep current object index (header) page in fs->work buffer
int32_t spiffs_object_modify(spiffs_fd *fd, u32_t offset, uint8_t *data, u32_t len) {
    spiffs *fs = fd->fs;
    int32_t res = SPIFFS_OK;
    u32_t written = 0;
    ...
    while (res == SPIFFS_OK && written < len) {
        ...
        if (cur_objix_spix != prev_objix_spix) {
            ...
            if (written > 0) {
                ...
                if (prev_objix_spix == 0) {
                    ...
                    res = spiffs_page_move(fs, fd->file_nbr, (uint8_t *) objix, fd->obj_id, 0, cur_objix_pix,
                                           &new_objix_pix);
                    SPIFFS_DBG("modify: store previous modified objix page, %04x:%04x, written %i\n", new_objix_pix,
                               objix->p_hdr.span_ix, written);
                    SPIFFS_CHECK_RES(res);
                    spiffs_cb_object_event(fs, fd, SPIFFS_EV_IX_UPD, fd->obj_id, objix->p_hdr.span_ix, new_objix_pix,
                                           0);
                }
            }
        ...
        if (cur_objix_spix == 0) {
            ...
            // get data page from object index page
            orig_data_pix = ((spiffs_page_ix *) ((uint8_t *) objix + sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(
                    fs, data_spix)];
        }
        ...
        if (page_offs == 0 && to_write == SPIFFS_DATA_PAGE_SIZE(fs)) {
            ...
            if (page_offs + to_write < SPIFFS_DATA_PAGE_SIZE(fs)) {
                // after modification
                res = spiffs_phys_cpy(fs, fd->file_nbr,
                                      SPIFFS_PAGE_TO_PADDR(fs, data_pix) + sizeof(spiffs_page_header) + page_offs +
                                      to_write,
                                      SPIFFS_PAGE_TO_PADDR(fs, orig_data_pix) + sizeof(spiffs_page_header) + page_offs +
                                      to_write,
...
int32_t spiffs_object_truncate(
    ...
    if (remove && new_size == 0) {
        ...
        res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_UPDT,
                         fd->file_nbr,
                         SPIFFS_PAGE_TO_PADDR(fs, fd->objix_hdr_pix) + offsetof(spiffs_page_header, flags),
                         sizeof(uint8_t),
                         (uint8_t *) &flags);
        SPIFFS_CHECK_RES(res);
    }
    ...
    while (cur_size > new_size) {
        ...
        if (cur_objix_spix == 0) {
            ...
            ((spiffs_page_ix *) ((uint8_t *) objix_hdr +
                                 sizeof(spiffs_page_object_ix_header)))[data_spix] = SPIFFS_OBJ_ID_FREE;
        } else {
            // get data page from object index page
            data_pix = ((spiffs_page_ix *) ((uint8_t *) objix + sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(fs,
                                                                                                                 data_spix)];
            ((spiffs_page_ix *) ((uint8_t *) objix + sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(fs,
                                                                                                      data_spix)] = SPIFFS_OBJ_ID_FREE;
        }
        ...
        if (cur_size - SPIFFS_DATA_PAGE_SIZE(fs) >= new_size) {
            ...
            if (cur_objix_spix == 0) {
                // update object index header page
                ((spiffs_page_ix *) ((uint8_t *) objix_hdr +
                                     sizeof(spiffs_page_object_ix_header)))[data_spix] = new_data_pix;
...
static int32_t spiffs_obj_lu_find_free_obj_id_bitmap_v(spiffs *fs, spiffs_obj_id id, spiffs_block_ix bix, int ix_entry,
    ...
    if (id != SPIFFS_OBJ_ID_FREE && id != SPIFFS_OBJ_ID_DELETED) {
        ...
        uint8_t *conflicting_name = (uint8_t *) user_p;
        ...
        if (conflicting_name && (id & SPIFFS_OBJ_ID_IX_FLAG)) {
            ...
            spiffs_page_object_ix_header objix_hdr;
            res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ,
                             0, SPIFFS_PAGE_TO_PADDR(fs, pix), sizeof(spiffs_page_object_ix_header),
                             (uint8_t *) &objix_hdr);
            SPIFFS_CHECK_RES(res);
            if (objix_hdr.p_hdr.span_ix == 0 &&
                (objix_hdr.p_hdr.flags & (SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_IXDELE)) ==
...
static int32_t spiffs_obj_lu_find_free_obj_id_compact_v(spiffs *fs, spiffs_obj_id id, spiffs_block_ix bix, int ix_entry,
                                                      u32_t user_data, void *user_p) {
    (void) user_data;
    if (id != SPIFFS_OBJ_ID_FREE && id != SPIFFS_OBJ_ID_DELETED && (id & SPIFFS_OBJ_ID_IX_FLAG)) {
        ...
        res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ,
                         0, SPIFFS_OBJ_LOOKUP_ENTRY_TO_PADDR(fs, bix, ix_entry), sizeof(spiffs_page_object_ix_header),
                         (uint8_t *) &objix_hdr);
        if (res == SPIFFS_OK && objix_hdr.p_hdr.span_ix == 0 &&
            ...
            if (state->conflicting_name &&
                strcmp((const char *) state->conflicting_name, (char *) objix_hdr.name) == 0) {
                return SPIFFS_ERR_CONFLICTING_NAME;
            }
...
int32_t spiffs_obj_lu_find_free_obj_id(spiffs *fs, spiffs_obj_id *obj_id, uint8_t *conflicting_name) {
    int32_t res = SPIFFS_OK;
        ...
        if (state.max_obj_id - state.min_obj_id <= (spiffs_obj_id) SPIFFS_CFG_LOG_PAGE_SZ(fs) * 8) {
            ...
            if (state.compaction != 0) {
                ...
                SPIFFS_DBG("free_obj_id: COMP select index:%i min_count:%i min:%04x max:%04x compact:%i\n", min_i,
                           min_count, state.min_obj_id, state.max_obj_id, state.compaction);
                ...
                if (min_count == 0) {
                    ...
                    *obj_id = min_i * state.compaction + state.min_obj_id;
                    ...
                    SPIFFS_DBG("free_obj_id: COMP SEL chunk:%04x min:%04x -> %04x\n", state.compaction,
                               state.min_obj_id, state.min_obj_id + min_i * state.compaction);
            ...
            memset(fs->work, 0, SPIFFS_CFG_LOG_PAGE_SZ(fs));
            res = spiffs_obj_lu_find_entry_visitor(fs, 0, 0, 0, 0, spiffs_obj_lu_find_free_obj_id_compact_v, 0, &state,
                                                   0, 0);
            if (res == SPIFFS_VIS_END) res = SPIFFS_OK;
            SPIFFS_CHECK_RES(res);
...
int32_t spiffs_fd_find_new(spiffs *fs, spiffs_fd **fd) {
    u32_t i;
    spiffs_fd *fds = (spiffs_fd *) fs->fd_space;
    for (i = 0; i < fs->fd_count; i++) {
        spiffs_fd *cur_fd = &fds[i];
